...
import { Entity, LngLat, PolygonEditConfig } from "../type";
export class StraightArrow {
    ...
    showArrowOnMap(positions: Cesium.Cartesian3[]) {
        ...
        let outline: any = true
        // if (this.config.borderStyle == 'dashed') {
        //     outline = new Cesium.PolylineDashMaterialProperty({
        //         dashLength: 16,
        ...
        this.handler.setInputAction((evt: Cesium.ScreenSpaceEventHandler.MotionEvent) => { //移动时绘制面
            ...
            if (this.positions.length >= 2) {
                if (!Cesium.defined(this.mainEntity)) {
                    this.positions.push(cartesian);
                    this.mainEntity = this.showArrowOnMap(this.positions);
    ...
    update(config: PolygonEditConfig) {
